...
BBS turns every model folder into a category in its form picker — that's where
"Modeller (emoticons)" comes from. The models themselves are **jar resources**
under `assets/bbs/assets/models/<category>/<model>/`, not config files: the
emoticons folders in `config/bbs/assets/models` are empty overlays and their
models still render. So EFMocap ships its actor the same way, inside its own
jar, and BBS picks the category up with nothing to install.

The actor (`assets/bbs/assets/models/efmocap/actor`) is generated by
`tools/gen_actor_model.py`. Its skeleton is copied bone-for-bone from BBS's own
emoticons rig (`emoticons/steve/default.bobj`):
...
Note the naming is BBS's, not anatomy's: `body` is the waist and `low_body` the
chest carrying the arms and head. Two things follow from matching it. The limbs
are already split at the elbow and knee, so Epic Fight's elbow/knee rotations
get their own bone instead of collapsing into the shoulder and hip. And because
the names are BBS's rather than ours — down to the asymmetric `low_leg_right` —
one exported animation drives this actor and BBS's emoticons models alike. The
torso is split at the waist too, so bending the chest moves the mesh rather than
just carrying the limbs around. The UV formula in that script is checked against
BBS's own alex model on the bones whose geometry is identical.

Skinned `.bobj` meshes are what BBS's emoticons models actually ship as, so the
installer copies those (and `.bbmodel` sources) alongside `.bbs.json` and PNGs.

`config/bbs/assets/models/efmocap/actor` is the writable overlay beside it:
Epic Fight animations converted with `/efmocap bbsexport` are written there, so
they sit next to the model where BBS's loader expects them. Anything dropped
into `config/efmocap/bbs-models` is mirrored into the category too
(`bbs/BBSAssets`), for models authored outside the mod. The mirror only writes
files that changed and never deletes, because BBS's own editor saves there.
