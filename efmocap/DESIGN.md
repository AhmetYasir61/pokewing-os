...
`tools/gen_actor_model.py`. Its bones deliberately reuse BBS's player names —
anchor, low_body, body, torso, head, `left_arm`/`right_arm`,
`left_leg`/`right_leg` — so the Epic Fight retarget map lands on it unchanged,
and each limb additionally splits into a child segment (`right_forearm`,
`right_shin`, plus empty `right_hand`/`right_foot` tips) so Epic Fight's elbow
and knee rotations have their own bone instead of collapsing into the shoulder
and hip. The UV formula in that script is checked against BBS's own alex model
on the bones whose geometry is identical.
